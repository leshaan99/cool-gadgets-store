 ...
 if (isset($_GET['user_id'])) {
    ...
    if ($result_set) {
        ...
        if (mysqli_num_rows($result_set)>0) {
            ...
            $user_id100 = $result['customer_id'];
            $name100 = $result['customer_name'];
            $email100 = $result['customer_email'];
            $password100= $result['customer_pass'];
 ...
 if(isset($_POST['submit'])) {
    ...
    //$result_set = mysqli_query($conn, $query);
       ...
       // $user_type = mysqli_real_escape_string($con,$user_type11);
        //$id = mysqli_real_escape_string($con, $user_id11);

        $insert = "INSERT INTO admins(admin_name, admin_email, password) VALUES('$name111','$email111','$password111')";
